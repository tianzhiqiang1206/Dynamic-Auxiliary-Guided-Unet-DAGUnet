...
class WAM(nn.Module):
    ...
    def forward(self, x_SIV, x_SIC):
        assert x_SIV.size(1) == x_SIC.size(1), f"Channel mismatch: {x_SIV.shape} vs {x_SIC.shape}"

        x_shared = self.A_in_SIV * x_SIV + self.A_in_SIC * x_SIC
...
class HISUnet(nn.Module):
    def __init__(self, in_channels, out_channels_SIV=2, out_channels_SIC=1, out_channels_SIT=1, predict_days=7):
        super().__init__()
        ...
        self.down1_SIT = DoubleConvNoPool(init_channels, base_channels*2)
        self.down2_SIT = Down(base_channels*2, base_channels*4, use_pool=True)
        self.down3_SIT = Down(base_channels*4, base_channels*8, use_pool=True)
        self.down4_SIT = Down(base_channels*8, base_channels*16, use_pool=True)

        self.up1_SIT = Up(base_channels*16, base_channels*8)
        self.up2_SIT = Up(base_channels*8, base_channels*4)
        self.up3_SIT = Up(base_channels*4, base_channels*2)
        self.outc_SIT = nn.Conv2d(base_channels*2, out_channels_SIT * predict_days, kernel_size=1)
