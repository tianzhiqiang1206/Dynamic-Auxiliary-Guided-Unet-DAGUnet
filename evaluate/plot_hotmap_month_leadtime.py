...
def plot_sic_heatmap(data, title, cmap, vmin=None, vmax=None, fmt='.4f'):
    ...
    # 设置标题和标签
    # plt.title(f'2019年SIC{title}热力图（Day1-Day7 × 1-12月）', fontsize=14, pad=20)
...
for year in year_list:
    ...
    for day_inx in range(1,8):
        ...
        for month_inx in range(1,13):       
            # start_inx = month_indexs[month_inx-1][0]
            # end_inx = month_indexs[month_inx-1][1]
            # # 对SIC和SIV数据进行逆归一化
            # SIC_pred = inverse_normalize(ds[f'SIC_pred_day{day_inx}'][start_inx:end_inx], sic_min, sic_max)
            # SIC_true = inverse_normalize(ds[f'SIC_true_day{day_inx}'][start_inx:end_inx], sic_min, sic_max)
            # SIV_U_pred = inverse_normalize(ds[f'SIV_u_pred_day{day_inx}'][start_inx:end_inx], siv_u_min, siv_u_max)
            # SIV_U_true = inverse_normalize(ds[f'SIV_u_true_day{day_inx}'][start_inx:end_inx], siv_u_min, siv_u_max)
            # SIV_V_pred = inverse_normalize(ds[f'SIV_v_pred_day{day_inx}'][start_inx:end_inx], siv_v_min, siv_v_max)
            # SIV_V_true = inverse_normalize(ds[f'SIV_v_true_day{day_inx}'][start_inx:end_inx], siv_v_min, siv_v_max)
    ...
    plot_sic_heatmap(SIC_MAE_array, 
                    title='Average MAE', 
                    cmap=purple_white_green,  # 反向蓝色系，越小越亮
